...
def get_orcaout_text(log_out: str) -> dict:
    ...
    """
    # name, _, charge, _, multiplicity = log_out.stem.split("_")

    with open(log_out) as f:
        orca_log = f.read()
    orca_log = orca_log.split("$$$$$$$$$$$$$$$$  JOB NUMBER  2 $$$$$$$$$$$$$$")[0]
...
def pareser():
    ...
    my_parser.add_argument('-names','--names',
                            metavar='names',
                            type=str,
                            help='file with the names of the molecules',
                            action='store',
                            default=None
                            )
...
if __name__ == '__main__':
    ...
    log("Parsing results")
    for idx, folder_path in enumerate(names): 
        # log(folder.name)
        ...
        log_file = folder.parent / (folder.stem + ".log")
        opt_file = folder.parent / (folder.stem + ".opt")
        xyz_file = folder.parent / (folder.stem + ".xyz")
