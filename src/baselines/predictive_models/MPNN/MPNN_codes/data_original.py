...
def build_dataset(dataset, target):
    processed = list()
    ...
    for idx in tqdm(range(len(dataset))):
        try:
            mol_smiles = dataset.loc[idx]['SMILES']
            mol = Chem.MolFromSmiles(mol_smiles)
            mol = Chem.AddHs(mol)
            mol_smiles = Chem.MolToSmiles(mol)
            mol_graph = get_graph_from_smile(mol_smiles, idx)

            label = torch.tensor(dataset.loc[idx][target], dtype=torch.float)

            processed.append([mol_graph, label])
        except:
            continue
...
if __name__ == "__main__":
    p = 'MP'  # elongation / strength / modulus / MP

    df = pd.read_csv(f'data/raw/ep_meltingpoint.csv', sep=",")
    # Chr : (Chromophore, Solvent, Absorption max (nm)/ Emission max (nm)/ Lifetime (ns))
    processed_data, dataset = build_dataset(df, p)
    torch.save(processed_data, f"./data/processed/ep_{p}.pt")
