...
class gat_layer(nn.Module):
    ...
    def __init__(self,
        ...
        for layer in range(num_step_message_passing):
            self.conv.append(GATConv(node_hidden_dim, int(node_hidden_dim/n_heads), heads=n_heads,
                                     edge_dim=edge_input_dim, add_self_loops=False))
    ...
    def forward(self, g):
        ...
        for i in range(self.num_step_message_passing):
            out = torch.relu(self.conv[i](out, edge_index, edge_attr))
            out = self.lns[i](out)
...
class GAT(nn.Module):
    def __init__(self,
        ...
        self.mol_gather = gat_layer(self.node_input_dim, self.edge_input_dim,
