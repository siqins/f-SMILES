...
def predict(model, dataset, feature_dicts, batch_size, device='cpu', per_task_output_units_num: int=2):
    ...
    for idx, task in enumerate(tasks):
        ...
        y_pred_prob_list[task] = np.array([])
    ...
    for counter, batch in enumerate(batch_list):
        ...
        for idx, task in enumerate(tasks):
            y_pred = mol_prediction[:, idx * per_task_output_units_num: (idx + 1) * per_task_output_units_num]
            y_pred = F.softmax(y_pred, dim=-1).data.cpu().numpy()
            y_pred_classes = np.argmax(y_pred, axis=1)  # 预测类别
            y_pred_max_probs = np.max(y_pred, axis=1)  # 最大概率值
            y_pred_list[task] = np.concatenate([y_pred_list[task], y_pred_classes])
            y_pred_prob_list[task] = np.concatenate([y_pred_prob_list[task], y_pred_max_probs])
...
def train_AFP(
    ...
    for idx, task in enumerate(tasks):
        weights_task = []
        for i in range(per_task_output_units_num):
            class_df = all_dataset[all_dataset[task] == i]
            weights_task.append(len(all_dataset) / (len(class_df) + 1e-5))
        weights.append(weights_task)
    ...
    for epoch in range(epochs):
        train_roc, train_loss = eval(
            ...
            device=device,
            per_task_output_units_num=per_task_output_units_num,
        )
...
def predict_AFP(
    ...
    probs.columns = [task + 'prob' for task in tasks]

    predictions = pd.concat([pred_dataset, predictions, probs], axis=1)
    return predictions
...
if __name__ == '__main__':
    ...
    train_dataset['task'] = [random.choice([0,1,2]) for _ in range(len(smis))]

    test_dataset = pd.DataFrame()
    test_dataset['smiles'] = smis
    test_dataset['task'] = [random.choice([0,1,2]) for _ in range(len(smis))]

    train_AFP(train_dataset=train_dataset, test_dataset=test_dataset, epochs=3, save_path='saved_models',
              per_task_output_units_num=3)

    print()

    print(eval_AFP(eval_dataset=test_dataset,
                   model_path='saved_models/model_cla_mc.pt',
