...
import os
from AttentiveFP_codes.Featurizer import *
import pickle
import time
...
#         smiles = Chem.MolToSmiles(Chem.MolFromSmiles(smiles))
        try:
            ...
            a = smiles_to_fingerprint_array.keys()
...
def save_smiles_dicts(smilesList,filename: str=None):
    #first need to get the max atom length
    max_atom_len = 0
    max_bond_len = 0
    num_atom_features = 0
    ...
    # smi = 'CCCCC(CC)Cc1ccc(/C2=c3/cc/c4n3[Zn]n3c(ccc3/C(c3ccc(CC(CC)CCCC)s3)=C3/C=CC(=N3)/C=4C#Cc3ccc(C4=C5C(=O)N(CC(CC)CCCC)C(c6cccs6)=C5C(=O)N4CC(CC)CCCC)s3)/C(C#Cc3ccc(C4=C5C(=O)N(CC(CC)CCCC)C(c6cccs6)=C5C(=O)N4CC(CC)CCCC)s3)=C3/C=CC2=N3)s1'
    # a = smi in smilesList
    smiles_to_fingerprint_features = gen_descriptor_data(smilesList)
    # b = smiles_to_fingerprint_features.keys()
    # c = smi in b
...
def get_smiles_array(smilesList, feature_dicts):
    ...
    for smiles in smilesList:
        ...
        x_mask.append(feature_dicts['smiles_to_atom_mask'].get(smiles, feature_dicts['smiles_to_atom_mask']))
        x_atom.append(feature_dicts['smiles_to_atom_info'][smiles])
        x_bonds.append(feature_dicts['smiles_to_bond_info'][smiles])
        x_atom_index.append(feature_dicts['smiles_to_atom_neighbors'][smiles])
        x_bond_index.append(feature_dicts['smiles_to_bond_neighbors'][smiles])
...
if __name__ == '__main__':
    ...
    smi = 'C1=CS23(C=C1)(C=CC(C1=COCC=C1)=C2)c1cc[nH]c1-c1[nH]ccc13'
    # smi = 'C1=CS23(N=CC=N2)(N=CC=N3)c2cc3c(cc21)Sc1ccccc1S3'
    sm = 'C1=CS23(C=C1)(C=CC(c1cc[nH]c1)=C2)C1=C(Cc2cscc2C1)c1cc[nH]c13'
    # graph_from_smiles(smi)
    ...
    sm1 = 'C1=CS23(C=C1)(C=CC(c1cc[nH]c1)=C2)c1[nH]ccc1-c1c3[nH]c2cscc12'
    sm2 = 'C1=CS23(C=C1)(C=CC(c1ccsc1)=C2)c1ccccc1-c1c3[nH]c2c1=CCC=2'
    sm3 = 'C1=CS23(C=C1)(C=CC(c1cc[nH]c1)=C2)c1cc2c(cc1-c1cc[nH]c13)Sc1ccccc1S2'
    sm4 = 'C1=CS23(C=C1)(C=CC=C2)C1=C(Cc2cscc2C1)c1c(-c2ccsc2)csc13'
    sm5 = '[Se]'
    a = save_smiles_dicts([sm1, sm2, sm3, sm4])
    print(a)