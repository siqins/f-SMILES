...
from FraGAT_codes.Model import *
from FraGAT_codes.Dataset import *
from FraGAT_codes.Valid import *
from FraGAT_codes.Metrics import *
from FraGAT_codes.Config import *
from FraGAT_codes.Utils import *
...
def train_and_evaluate(opt):
    ...
    if opt.args['train_dataset'] is None:
        ...
        Trainset = MolDatasetTrain(opt.args['train_dataset'], opt)
        if opt.args['valid_dataset'] is not None:
            Validset = MolDatasetEval(opt.args['valid_dataset'], opt)
        if opt.args['test_dataset'] is not None:
            Testset = MolDatasetEval(opt.args['test_dataset'], opt)
        else:
            Testset = Validset
    print(net)
    ...
    while epoch < opt.args['MaxEpoch']:
        ...
        for ii, data in enumerate(trainloader):
            ...
            if (ii + 1) % opt.args['ValidRate'] == 0:
                if opt.args['ClassNum'] == 1:
                    result = Validation(validloader, net, [MAE(), RMSE()], opt, device)
                    stop_flag, best_ckpt, best_value = saver.SaveModel(net, optimizer, epoch, result)
                else:
                    result = Validation(validloader, net, [AUC(), ACC()], device)
                    stop_flag, best_ckpt, best_value = saver.SaveModel(net, optimizer, epoch, result)

        if opt.args['ClassNum'] == 1:
            result = Validation(validloader, net, [MAE(), RMSE()], opt, device)
            print("running on test set.")
            testresult = Validation(testloader, net, [MAE(), RMSE()], opt, device)

            stop_flag, best_ckpt, best_value = saver.SaveModel(net, optimizer, epoch, result, testresult)
            epoch += 1
        else:
            result = Validation(validloader, net, [AUC(), ACC()], opt, device)
            print("running on test set.")
            testresult = Validation(testloader, net, [AUC(), ACC()], opt, device)
