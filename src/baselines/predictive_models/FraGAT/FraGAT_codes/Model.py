...
from FraGAT_codes.ClassifierModel import DNN
# from FraGAT_codes.NaiveGCN import NaiveGCN
from FraGAT_codes.AttentiveFP import *
...
class MolPredFragFPv8(nn.Module):
    ...
    def forward(self, Input):
        ...
        pad_node_feature = t.zeros(batch_size, FP_size).to(device)
    ...
    def Input_cuda(self, Input):
        ...
        if not self.training:
            ...
            atom_features = atom_features.squeeze(dim=0).to(device)
            bond_features = bond_features.squeeze(dim=0).to(device)
            atom_neighbor_list_origin = atom_neighbor_list_origin.squeeze(dim=0).to(device)
            bond_neighbor_list_origin = bond_neighbor_list_origin.squeeze(dim=0).to(device)
            atom_mask_origin = atom_mask_origin.squeeze(dim=0).to(device)

            atom_neighbor_list_changed = atom_neighbor_list_changed.squeeze(dim=0).to(device)
            bond_neighbor_list_changed = bond_neighbor_list_changed.squeeze(dim=0).to(device)
            frag_mask1 = frag_mask1.squeeze(dim=0).to(device)
            frag_mask2 = frag_mask2.squeeze(dim=0).to(device)
            bond_index = bond_index.squeeze(dim=0).to(device)

            JT_bond_features = JT_bond_features.squeeze(dim=0).to(device)
            JT_atom_neighbor_list = JT_atom_neighbor_list.squeeze(dim=0).to(device)
            JT_bond_neighbor_list = JT_bond_neighbor_list.squeeze(dim=0).to(device)
            JT_mask = JT_mask.squeeze(dim=0).to(device)
