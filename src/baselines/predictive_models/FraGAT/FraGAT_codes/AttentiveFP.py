...
class AttentionCalculator(nn.Module):
    ...
    def forward(self, FP_align, atom_neighbor_list):
        ...
        device = attend_mask.device
        attend_mask = attend_mask.type(t.FloatTensor).unsqueeze(-1).to(device)
        ...
        softmax_mask = softmax_mask.type(t.FloatTensor).unsqueeze(-1).to(device)
...
class AttentiveFP_mol(nn.Module):
    ...
    def forward(self, atom_FP, atom_mask):
        ...
        device = mol_softmax_mask.device
        mol_softmax_mask = mol_softmax_mask.type(t.FloatTensor).to(device)
        # [batch, atom_length, 1]
