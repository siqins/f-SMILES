...
from FraGAT_codes.ChemUtils import *
import rdkit.Chem as Chem
import re
import random
from FraGAT_codes.Featurizer import *
from FraGAT_codes.Transformer import *
from FraGAT_codes.Splitter import *
from FraGAT_codes.Checker import *
...
class SingleTaskFileParser(BasicFileParser):
    def __init__(self):
        super().__init__()
...
class MolDatasetCreator(object):
    ...
    def __init__(self, opt):
        ...
        self.stfile_parser = SingleTaskFileParser()
        self.SplitterList = {
    ...
    def CreateDatasets(self):
        if self.opt.args['train_dataset'] is None:
            ...
            print("Parsing lines...")
            parser = self.FileParserList.get([self.opt.args['ExpName']], self.stfile_parser)
            ...
            raw_dataset = self.opt.args['train_dataset']
        ...
        if self.opt.args['Splitter'] and self.opt.args['train_dataset'] is None:
        ...
        elif self.opt.args['train_dataset'] is not None:
            trainset = self.opt.args['train_dataset']
            if self.opt.args['test_dataset'] is not None:
                validset = self.opt.args['valid_dataset']
                testset = self.opt.args['test_dataset']
                sets = (trainset, validset, testset)
            elif self.opt.args['valid_dataset'] is not None and self.opt.args['test_dataset'] is None:
                validset = self.opt.args['valid_dataset']
                sets = (trainset, validset)
            else:
                sets = (trainset)
        else:
