...
emd_path = os.path.join(path, 'initial/ele2emb.pkl')
ele2emb = pickle.load(open(emd_path,'rb'))
def ele_features(ele):
    fele = ele2emb[ele]
    return fele.tolist()

emd_path = os.path.join(path, 'initial/rel2emb.pkl')
rel2emb = pickle.load(open(emd_path,'rb'))
def relation_features(e1,e2):
    frel = rel2emb[(e1,e2)]
    return frel.tolist()

emd_path = os.path.join(path, 'chemprop/data/funcgroup.txt')
...
emd_path = os.path.join(path, 'initial/fg2emb.pkl')
fg2emb = pickle.load(open(emd_path, 'rb'))
def match_fg(mol):
    fg_emb = [[1] * 133]
    pad_fg = [[0] * 133]
    for sm in smart:
