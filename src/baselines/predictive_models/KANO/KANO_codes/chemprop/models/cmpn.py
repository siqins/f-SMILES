...
from KANO_codes.chemprop.features import BatchMolGraph,get_atom_fdim, get_bond_fdim, mol2graph
from KANO_codes.chemprop.nn_utils import index_select_ND, get_activation_function
import math
...
class CMPNEncoder(nn.Module):
    ...
    def forward(self, step, mol_graph, features_batch=None) -> torch.FloatTensor:
        ...
        if self.args.step == 'functional_prompt':
            ...
            target_index = torch.LongTensor(target_index).to(device)
            fg_hiddens = scatter_add(f_fgs, target_index, 0)
            fg_hiddens_atom = torch.repeat_interleave(fg_hiddens, torch.tensor(atom_num).to(device), dim=0)
            fg_out = torch.zeros(1, 133).to(device)
