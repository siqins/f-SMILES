...
def add_atom(mol, rxn_smarts_list, rxn_smarts_ring_list, rxn_smarts_make_ring, p, p_ring, p_make_ring):
  if np.random.random() < 1.0: # probability of adding ring atom
    ...
    if not mol.HasSubstructMatch(Chem.MolFromSmarts('[r3,r4,r5]'))\
      ...
      if np.random.random() < 0.5: # probability of starting a fused ring
...
def generate_mol(smiles,max_atoms,average_size,size_stdev, rxn_smarts_list, rxn_smarts_ring_list, rxn_smarts_make_ring, p, p_ring, p_make_ring):
  #np.random.seed(0)
  ...
  while num_atoms < max_atoms and count < max_atoms:
    count += 1
    mol, smiles = add_atom(mol, rxn_smarts_list, rxn_smarts_ring_list, rxn_smarts_make_ring, p, p_ring, p_make_ring)
    num_atoms = mol.GetNumAtoms()
