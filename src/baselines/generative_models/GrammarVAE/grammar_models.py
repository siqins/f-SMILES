...
class ZincGrammarModel(object):
    ...
    def encode(self, str_list, device='cpu'):
        ...
        return self.vae.encoder(torch.from_numpy(one_hot).to(device))[0]
    ...
    def _sample_using_masks(self, unmasked):
        ...
        eps = 1e-100
        unmasked = unmasked.data.cpu().numpy()
    ...
    def decode(self, z, device='cpu'):
        """ Sample from the grammar decoder """
        assert z.ndim == 2
        batch_size = z.shape[0]
        z = z if torch.is_tensor(z) else torch.from_numpy(z)
        z = z.float()
        h1, h2, h3 = self.vae.decoder.init_hidden(batch_size, device)
