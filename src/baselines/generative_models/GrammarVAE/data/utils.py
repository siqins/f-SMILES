...
def smiles_to_one_hot(smiles):
    ...
    print('parse_trees --')
    parse_trees = [next(parser.parse(t)) for t in tqdm(tokens, file=sys.stdout)]
    print('productions_seq --')
    productions_seq = [tree.productions() for tree in tqdm(parse_trees, file=sys.stdout)]
    print('indices --')
    indices = [np.array([prod_map[prod] for prod in entry], dtype=int) for entry in tqdm(productions_seq, file=sys.stdout)]
    max_len = np.max([len(indice) for indice in indices])
