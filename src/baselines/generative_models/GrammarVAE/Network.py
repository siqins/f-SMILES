...
class Decoder(nn.Module):
    ...
    def init_hidden(self, batch_size, device='cpu'):
        h1 = Variable(torch.zeros(1, batch_size, self.hidden_n), requires_grad=False).to(device)
        h2 = Variable(torch.zeros(1, batch_size, self.hidden_n), requires_grad=False).to(device)
        h3 = Variable(torch.zeros(1, batch_size, self.hidden_n), requires_grad=False).to(device)
        return h1, h2, h3
...
class EncoderZinc(nn.Module):
    def __init__(self, hidden_n=100, input_channels=80, linear_input=4510):
...
class GrammarVariationalAutoEncoder(nn.Module):
    def __init__(self, model_name='eq_grammar', rules=None):
        ...
        if model_name == 'mol_grammar':
            ...
            self.decoder = Decoder(input_size=56, hidden_n=56, output_feature_size=len(rules), max_seq_length=436)
    ...
    def forward(self, x):
        ...
        h1, h2, h3 = self.decoder.init_hidden(batch_size, device=x.device)
