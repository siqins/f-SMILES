...
def preprocess(train_path, save_path):
    ...
    # fragments = fragment_mols(smis, convert=True, method='default')  # use custom fragmentation technique
    # fragments = fragment_mols(smis, convert=True, method='mmpa')  # use MMPA fragmentation fraggle
    fragments = fragment_mols(smis, convert=True, method='fraggle')  # use fraggle fragmentation
    ...
    print(grammar_fragment.vocab)
    grammar_fragment.to_file(os.path.join(os.path.dirname(save_path), f'grammar_fragment.txt'))

    dataset = [grammar_fragment.full_encoder(Chem.MolFromSmiles(smi)) for smi in tqdm(smis)]
    alphabet = get_alphabet_from_selfies(dataset)
    ...
    pad_to_len = max(len_selfies(s) for s in dataset)  # 5
...
def train(data_path, save_path='experiments', epochs=200):
...
def sample(model_path, save_path, batch_size=250, sample_size=100, latent_dimension=25):
    ...
    grammar_fra = GroupGrammar.from_file('experiments/grammar_fragment.txt')
    ...
    for __ in range(sample_size):
        ...
        smi = grammar_fra.decoder(molecule)
        smi = Chem.MolToSmiles(smi)
        print(__, smi)
        smis.append(smi)

    df = pd.DataFrame(smis, columns=['smiles'])
    df.to_csv(os.path.join(save_path, 'samples.csv'), index=False)
...
if __name__ == '__main__':
    ...
    train(data_path=save_path, save_path='experiments', epochs=500)
