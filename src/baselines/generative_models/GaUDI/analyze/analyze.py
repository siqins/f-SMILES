...
def analyze_validity_for_molecules(molecule_list, tol=0.1, dataset="cata"):
    ...
    for i, (x, atom_type) in enumerate(molecule_list):
        ...
        connected_list.append(validity_results["connected"])
    ...
    validity_dict = {
        ...
        "molecule_connected_bool": connected_list,
    }
...
def analyze_rdkit_validity_for_molecules(
    ...
    with tqdm(molecule_list, unit="mol") as tq:
        for i, (x, rings_type) in enumerate(tq):
            if molecule_connected_bool:
                # check the connection of molecule
                if molecule_connected_bool[i]:
                    try:
    ...
    if calc_novelty:
        df_train = get_splits(
            Namespace(dataset=dataset, target_features=None, max_nodes=13)
        )[0]
