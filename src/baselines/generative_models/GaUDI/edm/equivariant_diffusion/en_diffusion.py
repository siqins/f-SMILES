...
from edm.equivariant_diffusion import utils
import numpy as np
import math
import torch
from edm.egnn import models
...
def gaussian_KL_for_dimension(q_mu, q_sigma, p_mu, p_sigma, d):
    ...
    Returns:
        The KL distance, summed over all dimensions except the batch dim.
    """
    if len(q_sigma.size()) == 0:
        q_sigma = q_sigma.reshape(-1)
    if len(p_sigma.size()) == 0:
        p_sigma = p_sigma.reshape(-1)
...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def __init__(
        ...
        if noise_schedule == "learned":
            self.gamma = GammaNetwork().to(device)
        else:
        ...
        self.register_buffer("buffer", torch.zeros(1).to(device))
    ...
    def sample_p_xh_given_z0(self, z0, node_mask, edge_mask, context, fix_noise=False):
        ...
        gamma_0 = self.gamma(zeros).to(z0.device)
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always):
        ...
        gamma_s = self.inflate_batch_array(self.gamma(s), x).to(x.device)
        gamma_t = self.inflate_batch_array(self.gamma(t), x).to(x.device)
        ...
        if t0_always:
            ...
            gamma_0 = self.inflate_batch_array(self.gamma(t_zeros), x).to(x.device)
    ...
    def sample_guidance(
        ...
        # Finally sample p(x, h | z_0).
        x, h = self.sample_p_xh_given_z0(
            z, node_mask, edge_mask, context=None, fix_noise=fix_noise
        )
    ...
    def sample_p_zs_given_zt_eps(
        self, eps, s, t, zt, node_mask, edge_mask, context, fix_noise=False
    ):
        """Samples from zs ~ p(zs | zt). Only used during sampling."""
        gamma_s = self.gamma(s).to(s.device)
        gamma_t = self.gamma(t).to(s.device)
        ...
        # Sample zs given the paramters derived from zt.
        # zs = self.sample_normal(mu, sigma, node_mask, fix_noise)
        eps = remove_mean_with_mask(eps * node_mask, node_mask)
        zs = mu + sigma * eps
    ...
    def sample_chain(
        ...
        """
        Draw samples from the generative model,
        keep the intermediate states for visualization purposes.
        """
        z = self.sample_combined_position_feature_noise(
            n_samples, n_nodes, node_mask, std=std
        )
