...
class Mol:
    def __init__(self, _atoms):
        if isinstance(_atoms[0], Atom):
            ...
            i = 0
            for atom in _atoms:
                self.atoms.append(Atom(i, atom[0], atom[1], atom[2], atom[3], atom[4]))
                i += 1

    def __str__(self):
        length = math.floor(math.log10(len(self.atoms))) + 1
        return "\n".join(
            f"{atom.index:{length}}  {atom.element:2} {atom.x:8.5f} {atom.y:8.5f} {atom.z:8.5f} {atom.numH}"
...
class Atom:
    ...
    def __repr__(self):
        return (
            f"{self.index:4} {self.element:2} {self.x:8.5f} {self.y:8.5f} {self.z:8.5f} {self.numH}"
        )

    def __hash__(self):
        return hash(f"{self.index}{self.element}{self.x}{self.y}{self.z}{self.numH}")
