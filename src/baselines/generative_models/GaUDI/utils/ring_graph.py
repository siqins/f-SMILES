...
NO_ORIENTATION_RINGS = ["Bn"]
ORIENT_CARBON_RINGS = ['Cyc']
THREE_HETERO_RINGS_1 = ['Tr']
THREE_HETERO_RINGS_2 = ['Td', 'Sed']
...
def get_rings(_atoms: Sequence[Atom], _molgraph: nx.graph) -> Sequence[Ring]:
    ...
    in:
    _atoms: A list of Atoms with their xyz coordinates in Angstroms. 来源于建立的Mol类，采用mol.atoms作为输入,包含原子符号以及三维坐标。
    _cycles: A list of monocycles. Each monocycle is a list of atom indices. 来源于用分子键接信息建立的nx.graph
    ...
    for cycle in cycles:
        ...
        for atom in cycle:
            cycle_atoms.append(_atoms[atom].element)
        ...
        if knot_type in NO_ORIENTATION_RINGS:
            ...
            if knot_type in ORIENT_CARBON_RINGS:
                orientation = [_atoms[atom].get_coord() for atom in cycle if _atoms[atom].numH == 2]
            elif knot_type in THREE_HETERO_RINGS_1:
                orientation = [_atoms[atom].get_coord() for atom in cycle if _atoms[atom].element == 'N' and _atoms[atom].numH == 1]
            elif knot_type in THREE_HETERO_RINGS_2:
                orientation = [_atoms[atom].get_coord() for atom in cycle if _atoms[atom].element in ['S', 'Se']]
            else:
                orientation = [_atoms[atom].get_coord() for atom in cycle if _atoms[atom].element != "C"]
