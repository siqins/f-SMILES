...
def val_epoch(tag, cond_predictor, edm_model, dataloader, args, edm_args, t_fix=None):
    ...
    with torch.no_grad():
        ...
        for i, (x, node_mask, edge_mask, node_features, y) in enumerate(tqdm(dataloader)):
            x = x.to(args.device)
            y = y.to(args.device)
...
if __name__ == "__main__":
    ...
    pred_args = get_cond_predictor_args(
        rf"D:\applications\les apps\OneDrive\项目一\PAS\diffusion\prediction_summary\hetro-test"
    )
